						  ...
						  <?php
							...
							if(isset($_REQUEST['submit'])){
								if($_REQUEST['prd'] != ""){
									...
									$sql = mysqli_query($link,"select distinct Department,TestName,Amount from testdetails where Department='$prdname' order by id desc") or die(mysqli_error($link));
								}
								if($_REQUEST['prd1'] != ""){
									$phone = $_REQUEST['prd1'];
									$sql = mysqli_query($link,"select distinct Department,TestName,Amount from testdetails where TestName='$phone' order by id desc")or die(mysqli_error($link));
								}
								if($_REQUEST['prd'] != "" && $_REQUEST['prd1'] != ""){
									$prdname = $_REQUEST['prd'];
									$phone = $_REQUEST['prd1'];
									$sql = mysqli_query($link,"select distinct Department,TestName,Amount from testdetails where Department='$prdname' and TestName='$phone' order by id desc")or die(mysqli_error($link));
								}
							}else{
								$sql = mysqli_query($link,"select distinct Department,TestName,Amount from testdetails order by id desc")or die(mysqli_error($link));
							}
							if($sql){
