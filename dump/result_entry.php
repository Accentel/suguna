                      ...
                      <tbody>
						  ...
						  <?php
							...
							if(isset($_REQUEST['submit'])){
								...
								if($prdname != "" & $pname != ""){
									$sql = mysql_query("select distinct BillNo,Pname,Age,Sex from reportentry where BillNo='$prdname' and Pname='$pname' order by id desc");
								}elseif($prdname != ""){
									$sql = mysql_query("select distinct BillNo,Pname,Age,Sex from reportentry where BillNo='$prdname' order by id desc");
								
								}elseif($pname != ""){
									$sql = mysql_query("select distinct BillNo,Pname,Age,Sex from reportentry where Pname='$pname' order by id desc");
								
								}
							}else{
								$sql = mysql_query("select distinct BillNo,Pname,Age,Sex from reportentry where  user='$name' order by id desc");
							}
							if($sql){
								...
								while($row = mysql_fetch_array($sql)){
									$blno = $row[0];
									$sql1 = mysql_query("select * from bill1 where BillNO='$blno'");
									if($sql1){
                      ...
                      <tbody>
						  ...
						  <?php
							...
							if(isset($_REQUEST['submit'])){
								...
								$sql = mysql_query("select distinct BillNo,Pname,Age,Sex from reportentry order by id desc");
							}
							if($sql){
								$rows=mysql_num_rows($sql);
								if($rows > 0){
								while($row = mysql_fetch_array($sql)){
									$blno = $row[0];
									$sql1 = mysql_query("select a.BillNo,a.conce_type,b.BalanceAmount from bill a,bill1 b where a.BillNo=b.BillNo and b.BillNO='$blno'");
									if($sql1){
										$row1 = mysql_fetch_array($sql1);
										$bamt = $row1['BalanceAmount'];
										$ctype = $row1['conce_type'];
