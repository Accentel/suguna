            ...
            <tr>
                <td>
                    <table cellpadding="4" cellspacing="0" width="100%" border="1">
						...
						<?php
							$qry1=mysqli_query($link,"select Percentage from ddetails where DoctorName='$dname'")or die(mysqli_error($link));
							if($qry1){
                      ...
                      $tname=array('X RAY EACH','SCANNING','Breast scan single','Breast scan both','X-RAY SHOULDER AP','X-RAY FOREARM AP & LAT','X-RAY HAND AP & LAT','x-ray erect abdomen','NT SCAN','X-RAY CHEST AP(RIBS)','X -RAY CHEST AP LAT','X-RAY CHEST PA','X-RAY CHEST AP','X-RAY PNS','X-RAY COCCYX AP & LAT','X-RAY ORBIT AP & LAT','X-RAY ZYGOMATIC BONE','X-RAY MANDIBLE','X-RAY ADENOIDS','X-RAY NASAL BONE BOTH LAT',
						...
						$qry=mysqli_query($link,"select distinct a.BillNO,b.BillDate,a.PatientName,a.DoctorName,b.PaidAmount from bill1 b,bill a where a.BillNO=b.BillNO and b.BillDate between '$sdate1' and '$edate1' and a.DoctorName='$dname'  and a.TestName NOT IN ('" . implode("','",$tname) . "') ");
                        ...
                        <tr>
							...
							<table width="100%">
							<?php
									$sql2 = mysqli_query($link,"select distinct TestName from bill where BillNO='$bno'")or die(mysqli_error($link));
									if($sql2){
