...
xmlhttp.send();
	...
	<div id="conteneur">
		...
		<?php
			include("config.php");
			$bno = $_REQUEST['bno'];
			$sql = mysql_query("select BillNO, PatientName,Age,Sex,DoctorName,conce_type,ptype,sum(Total) as Total,sum(Discount) as Discount,sum(NetAmount) as NetAmount,sum(PaidAmount) as PaidAmount, sum(BalanceAmount) as BalanceAmount, remarks  from bill where BillNO = '$bno'");
			if($sql){
			...
			<table width="100%" border="0" cellpadding="4" cellspacing="0">
					...
					<table width="100%" id="TABLE1">
					  <thead>
					   ...
					   <?php 
						$sql2 = mysql_query("select * from bill where BillNO = '$bno'");
						if($sql2){
							while($row2 = mysql_fetch_array($sql2)){
							
					   ?>
					   <tr>
					   <td ><input type="text" class="text" name="tname[]" id="cname" value="<?php echo $row2['TestName'] ?>"  /></td>
						<td ><input type="text" class="text" name="cost[]" id="cost" value="<?php echo $row2['Amount'] ?>"  /></td>
					...
					<tr >
                        ...
                        <td align="left" >
                            <input type="text" name="conamt"  value="<?php echo $disc ?>" id="conamt" onkeyup="paidamt()" readonly class="text"/>
                        </td>
                    </tr>
					<tr >
                        <td align="right" >Net Amount :</td>
                        <td align="left" >
                            <input type="text" name="price"  value="<?php echo $net ?>" id="price" class="text" readonly />
                        </td>
						 <td align="right" >Paid Amount :</td>
                        <td align="left" >
                            <input type="text" name="paid"  value="<?php echo $paid ?>" onkeyup="paidamt()" readonly id="paid" class="text"/>
                        </td>
                    </tr>
					<tr >
