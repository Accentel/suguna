...
<html>
	...
	<body>
		...
		<?php
			//include("config.php");
			$bno = $_REQUEST['bno'];
			$sql = mysqli_query($link,"select  b.BillNO,b.BillDate,b.PatientName,b.Age,b.Sex,b.DoctorName,b.conce_type,b.phoneno,b.ptype,b1.Total,b1.Discount,b1.NetAmount,b1.PaidAmount,b1.BalanceAmount,b1.time from outerbill b,outerbill1 b1 where b.BillNO=b1.BillNO and  b.BillNO = '$bno'") or die(mysqli_error($link));
			if($sql){
		...
		?>
		  ...
		  <form name="myform" class="table" method="post" action="update_outerbill.php">
...
<?php } ?>
					...
					<table width="100%" id="TABLE1">
					  <thead>
					   ...
					   <?php 
						$sql2 = mysqli_query($link,"select * from outerbill where BillNO = '$bno'") or die(mysqli_error($link));
						if($sql2){
					   ...
					   ?>
					   <tr>
					   <td ><input type="text" class="text" name="tname[]" id="cname1"   value="<?php echo $row2['TestName'] ?>"  /></td>
						<td ><input type="text" class="text" name="cost[]" id="cost1" readonly value="<?php echo $row2['Amount'] ?>"  /></td>
						<td><a href="deletereport1.php?q=<?php echo $id  ?>&bno=<?php echo $bn ?>"><img src="images/Icon_Delete.png" height="21"/></a></td>
						</tr>
						<?php } } ?>
					 </table>
					...
					<tr >
                        ...
                        <td align="left" >
                            <input type="text"  name="tot" value="<?php echo $tot ?>" readonly id="tot" class="text"/>
                        </td>
						 <td align="right" >Discount :</td>
                        <td align="left" >
                            <input type="text"  name="conamt"  value="<?php echo $disc ?>" readonly id="conamt"  class="text"/>
                        </td>
                    </tr>
					<tr >
                        <td align="right" >Net Amount :</td>
                        <td align="left" >
                            <input type="text"  name="price"  value="<?php echo $net ?>"  readonly id="price" class="text"  />
                        </td>
						 <td align="right" >Paid Amount :</td>
                        <td align="left" >
                            <input type="text"  name="paid"  value="<?php echo $paid ?>" readonly id="paid" class="text"/>
                        </td>
                    </tr>
				<tr >
                        <td align="right" >Balance Amount :</td>
                        <td align="left" >
                            <input type="text" name="bal"  value="<?php echo $bal ?>" readonly  id="bal" class="text"/>
		 ...
		 <div align="center"><input type="submit" name="submit" value="Update" Class="butt"/> <input type="button" value="Close" Class="butt" onclick="window.location.href='new_lab_outerbill.php'"/></div>
