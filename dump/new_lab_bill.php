                      ...
                      <tbody>
						  ...
						  <?php
							...
							if(isset($_REQUEST['submit'])){
								...
								if($prdname != "" & $pname != ""){
									$sql = mysql_query("select distinct BillNO,PatientName,NetAmount,PaidAmount,BalanceAmount from bill1 where BillNO='$prdname' and PatientName='$pname' order by bid desc");
								}elseif($prdname != ""){
								...
								$sql = mysql_query("select  a.BillNO,a.PatientName,a.NetAmount ,a.PaidAmount,a.BalanceAmount,b.ptype,b.conce_type,b.BillNO  from bill1 a,bill b where a.BillNO=b.BillNO and a.user='$name'  GROUP BY  a.BillNO  order by a.bid desc LIMIT $start_from, ".$results_per_page);
							}
							if($sql){
		          ...
		          <table border="0" cellpadding="0" cellspacing="0" class="table table-bordered" id="expense_table" width="100%">
                      ...
                      <tbody>
						  ...
						  <?php
							...
							if (isset($_GET["page"])) { $page  = $_GET["page"]; } else { $page=1; };
							$start_from = ($page-1) * $results_per_page;
								$sql = mysql_query("select  a.BillNO,a.PatientName,a.NetAmount ,a.PaidAmount,a.BalanceAmount,b.ptype,b.conce_type,b.BillNO  from bill1 a,bill b where a.BillNO=b.BillNO     GROUP BY  a.BillNO  order by a.bid desc LIMIT $start_from, ".$results_per_page);
							}
							if($sql){
								$rows=mysql_num_rows($sql);
								if($rows > 0){
