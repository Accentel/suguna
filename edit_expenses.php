...
//include("config.php");
$id = $_REQUEST['id'];
$sql=mysqli_query($link,"select * from expensemstr where id=$id") or die(mysqli_error($link));
if($sql){
...
$sq=mysqli_query($link,"update expensemstr set date='$edate',name='$ename',amount=$eamt,purpose='$purp' where id=$id") or die(mysqli_error($link));
if($sq){
	print "<script>";
